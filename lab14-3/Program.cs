...
namespace lab14_3
{
    ...
    class ColdCallFileReader : IDisposable
    {
        ...
        public void ProcessNextPerson()
        {
            ...
            if (!isOpen)
            {
                throw new Exception("Attempt to access cold call file that is not open.  Call Open() first.");
            }
            try
            {
    ...
    public class SalesSpyFoundException : Exception
    {
        ...
        public SalesSpyFoundException(string message, Exception inner) : base(message, inner) { }
    }
}
